...
export default function Chu2Calculate({ onClose, baseInfo }) {
  ...
  const handleValuesChange = (_, values) => {
    ...
    if (oneThousandMeter) {
      ...
      const oneThousandMeterBonus =
        calculateOneThousandMeterBonusScore(oneThousandMeter);
    ...
    if (sitUps) {
      setScore((prev) => ({
        ...prev,
        sitUps: calculateSitAndReachScore(sitUps),
      }));
      // 计算仰卧起坐加分
  ...
  const totalScore = useMemo(() => {
    ...
    curScore,
    eightHundredMeterBonus,
    oneThousandMeterBonus,
    pullUpExceed,
    sitUpBonus,
  ]);
