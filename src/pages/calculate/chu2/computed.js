...
export function calculatePullUpScore(count) {
	...
	for (const standard of standards) {
		if (count >= (standard.min || 0) && (standard.max === undefined || count <= standard.max)) {
			return standard.score
		}
	}
	return 0
...
export function calculateStandingLongJumpScore(distance, isBoy) {
	const standards = isBoy ? boyScoreStandards.standingLongJump : girlScoreStandards.standingLongJump
	for (const standard of standards) {
		if (distance >= (standard.min || 0) && (standard.max === undefined || distance <= standard.max)) {
			return standard.score
		}
	}
	return 0
