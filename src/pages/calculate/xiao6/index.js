...
export default function Xiao6Calculate({ onClose, baseInfo }) {
  ...
  const handleValuesChange = (_, values) => {
    ...
    if (sitAndReach) {
      setScore((prev) => ({
        ...prev,
        sitAndReach: calculateSitAndReachScore(sitAndReach),
      }));
    } else {
      setScore((prev) => ({
        ...prev,
    ...
    if (jumpRope) {
      ...
      setJumpRopeExtraScore(
        calculateJumpRopeBonus(jumpRope, gender === "male")
      );
    } else {
      setScore((prev) => ({
        ...prev,
    ...
    if (eightTimesFiftyMeter) {
      setScore((prev) => ({
        ...prev,
        eightTimesFiftyMeter: calculateEightTimesFiftyMeterScore(
          eightTimesFiftyMeter,
          gender === "male"
        ),
      }));
    ...
    <>
      ...
      >
        ...
        {curScore?.jumpRope !== undefined && (
          <span className={scoreSpanStyle}>单项成绩: {curScore.jumpRope}</span>
        )}
        {jumpRopeExtraScore > 0 && (
          <span className={extraScoreSpanStyle}>
            额外加分: {jumpRopeExtraScore}
          </span>
        )}
        ...
        {curScore?.sitUps !== undefined && (
          <span className={scoreSpanStyle}>单项成绩: {curScore.sitUps}</span>
        )}
        ...
        {curScore?.eightTimesFiftyMeter !== undefined && (
          <span className={scoreSpanStyle}>
            单项成绩: {curScore.eightTimesFiftyMeter}
          </span>
        )}
      </Form>
      <Popup
